...
@section('title', __('monitoring.tab.visits'))

@php
    // The package controller paginates without with('user'), so reading $visit->user per
    // row would lazy load -- which AppServiceProvider forbids outside production. Load the
    // relation once for the whole page instead of patching the vendor controller.
    $visits->loadMissing('user');
@endphp
        ...
        <table class="w-full whitespace-nowrap text-small font-mono">
            <thead>
                <tr class="text-x-small uppercase tracking-wider text-muted border-b border-border">
                    <th class="text-left font-medium px-4 py-3">{{ __('monitoring.column.page') }}</th>
                    <th class="text-left font-medium px-4 py-3">{{ __('monitoring.column.user') }}</th>
                    <th class="text-left font-medium px-4 py-3">{{ __('monitoring.column.browser') }}</th>
            ...
            <tbody>
                @forelse ($visits as $visit)
                    <tr class="border-b border-border/40 hover:bg-elevated/30 transition-colors">
                        ...
                        <td class="px-4 py-3 text-muted">
                            {{ $visit->user->{config('user-monitoring.user.display_attribute')} ?? __('monitoring.guest') }}
                        </td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <form method="post" action="{{ route('user-monitoring.visits-monitoring-delete', $visit->id) }}"
                                  onsubmit="return confirm(@js(__('monitoring.confirm_delete.visits')))">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-12 text-center text-muted">{{ __('monitoring.empty.visits') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
