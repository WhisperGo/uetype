{{-- Official Chart.js logo (chartjs/Chart.js docs/.vuepress/public/logo.svg): a hexagonal
     frame around three data bands in the default Chart.js palette -- blue (#36A2EB),
     yellow (#FFCE56), pink (#FE6184) -- with a light-grey hex border (#E7E9ED). Reproduced
     verbatim from the source; the Simple Icons `chartdotjs` mark is monochrome only, so it
     can't carry the brand's colours. --}}
@props(['class' => 'w-4 h-4'])
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192" {{ $attributes->merge(['class' => $class]) }}>
