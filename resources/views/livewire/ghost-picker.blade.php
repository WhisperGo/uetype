    ...
    <x-modal name="ghost-picker" maxWidth="md" focusable>
        <div class="p-6 space-y-5">
            <div class="flex items-center justify-between">
                <h3 class="font-sans text-lg font-bold text-foreground">Choose a Ghost</h3>
                <button type="button" x-on:click="$dispatch('close-modal', 'ghost-picker')"
                    class="text-muted hover:text-foreground transition-colors" aria-label="Close">
            ...
            @if (! in_array($mainMode, ['time', 'words'], true))
                <p class="font-mono text-sm text-muted">Ghost Mode is only available for Time and Words.</p>
            @else
                {{-- ===== MY BEST ===== --}}
                <div>
                    <p class="font-mono text-xs uppercase tracking-widest text-muted mb-2">My Best</p>
                    @if ($this->myBest > 0)
                        <button type="button" wire:click="selectOpponent('own')"
                            ...
                            <span class="font-mono text-sm text-foreground">Your Best</span>
                            <span class="font-mono text-sm font-bold text-brand-bright">{{ rtrim(rtrim(number_format($this->myBest, 1), '0'), '.') }} wpm</span>
                        </button>
                    @else
                        <p class="font-mono text-xs text-muted italic">Complete a Time/Words test first to race your own best.</p>
                    @endif
                </div>

                {{-- ===== FRIENDS ===== --}}
                <div>
                    <p class="font-mono text-xs uppercase tracking-widest text-muted mb-2">Friends</p>
                    ...
                    @else
                        <p class="font-mono text-xs text-muted italic">No friends with a recorded time yet.</p>
                    @endif
                </div>
                ...
                <div>
                    <p class="font-mono text-xs uppercase tracking-widest text-muted mb-2">
                        Leaderboard &middot; {{ ucfirst($mainMode) }} {{ $subMode }}
                    </p>
                    ...
                    @else
                        <p class="font-mono text-xs text-muted italic">Leaderboard is empty for this mode.</p>
                    @endif
                </div>
